...
WebContents.prototype.takeHeapSnapshot = function (...args) {
  return new Promise((resolve, reject) => {
    const requestId = getNextId()
    this.send('ELECTRON_INTERNAL_RENDERER_TAKE_HEAP_SNAPSHOT', requestId, args)
    ipcMain.once(`ELECTRON_INTERNAL_BROWSER_TAKE_HEAP_SNAPSHOT_RESULT_${requestId}`, (event, error, result) => {
      if (error) {
        reject(errorUtils.deserialize(error))
      } else {
        resolve(result)
